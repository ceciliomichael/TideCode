...
- Terminal calls: start with `execute_terminal`; use `interact_terminal` to wait/read new output, type input, or send Ctrl+C to cancel. Stop when evidence is sufficient; do not poll running servers in an infinite loop.
