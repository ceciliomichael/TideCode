...
User-added models are managed from Settings → Models and are stored in one catalog per provider under `~/.echosphere/models`. Built-in provider catalogs remain read-only at runtime. Each user model owns its reasoning profile, so changing one model never changes another model from the same provider.
